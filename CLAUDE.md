...
El logotipo es **`chrm.`** y vive en `ui/Logo.jsx`, que es una copia del de `site/src/components/Logo.jsx` — si cambia uno, cambian los dos. Es un `<text>` en un SVG, no trazados, así que depende de que **Baloo 2** esté servida (`@font-face` en `index.css`, el .woff2 en `public/fonts/`, el mismo archivo que usa la landing). Dos cosas que parecen de más y no lo son: `font-display: block`, porque el `viewBox` de 186 está calculado contra las métricas de Baloo 2 y con el fallback a Verdana el logo sale cortado — antes que mostrarlo mal, no lo muestra; y `fill="currentColor"`, que es lo que lo hace servir en los dos temas sin dos archivos. `public/chrm-monochrome.svg` es el favicon de la landing y **no** sirve para esto: adentro tiene un `<image>` sin `href`.
